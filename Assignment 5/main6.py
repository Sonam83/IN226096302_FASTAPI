...
# ___________________- Ass 5 : Sort by category and then price _____________________________________
@app.get('/products/sort-by-category')
def sort_by_category():
    result = sorted(products,key=lambda p:(p['category'],p['price']))
    return {'products':result, 'total':len(result)}

# ________________ Ass 5 : sort-search-pageination together endpoint __________________________________
@app.get('/products/browse')
def browse_products(keyword:str = Query(None), sort_by:str=Query('price'),order:str=Query('asc'),page:int=Query(1,ge=1),
            ...
            return {'keyword':keyword,'sort_by':sort_by,'order':order,'page':page,'limit':limit,
            'total_found':total,'total_pages':-(- total//limit),'products':paged,}
...
# _______________________ Ass 5 : orders search ______________________________
...
# ___________________ Ass 5 : Orders paginate endpoint ______________________________
@app.get('/orders/page')
def get_orders_aged(page:int = Query(1,ge=1),limit:int=Query(3,ge=1,le=20),):
    start=(page-1)*limit
    return {'page':page, 'limit':limit,'total':len(orders),'total_pages': -(-len(orders)//limit),
